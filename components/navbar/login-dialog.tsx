...
const LoginDialog = () => {
    ...
    return (
        <Suspense fallback={<div />}>
            <Dialog>
                <Button variant={'secondary'} className='text-secondary rounded-xl'asChild>
...
export const Dialog1: React.FC<ModalProps> = ({ page, handleNext, handleBack }) => {
    ...
    return (
        <DialogContent className='border-none h-fit lg:w-7/12 md:w-4/5 text-secondary flex flex-row'>
            <Card className='bg-transparent flex flex-col text-secondary border-none' style={{ width: '90vw'}}>
                ...
                <CardContent className='bg-bg flex flex-col gap-2'>
                    ...
                    <Button onClick={ () => { handleNext(); } } variant={'secondary'} className='text-secondary w-full rounded-full' type="submit">Create account</Button>
                </CardContent>
            ...
            <Card className='bg-transparent w-1/2 flex flex-col text-secondary border-none' style={{ width: '50%'}}>
                ...
                <CardContent className='bg-bg flex flex-col gap-2 rounded-b-xl'>
                    <div className='flex flex-row gap-4 w-full justify-center items-center'>
                        ...
                        <Button onClick={() => handleNext()} variant={'secondary'} style={{ height: '27px' }} className='text-secondary w-3/4 rounded-lg' type="submit">Enter the App</Button>
                    </div>
                </CardContent>                
            </Card>      
    ...
    return (
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col w-full items-center space-y-2">
          ...
          />
          <Button variant={'secondary'} style={{ height: '27px' }} className='text-secondary w-full rounded-full' type="submit">Send OTP</Button>
        </form>
      </Form>
    )
  }
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col w-full items-center space-y-2 gap-4">
            ...
            <div className='flex flex-row gap-4 w-full justify-center items-center'>
                ...
                <Button onClick={() => handleComplete()} variant={'secondary'} style={{ height: '27px' }} className='text-secondary w-3/4 rounded-lg' type="submit">Create Profile</Button>
            </div>
          </form>
        </Form>
    )
      ...
      return (
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col w-full items-center space-y-2 gap-4">
           ...
           <div className='flex flex-row gap-4 w-full justify-center items-center'>
                <Button onClick={() => handlePrevious()}  className='text-secondary w-1/4 rounded-lg' type="submit"><ChevronLeftIcon width={150} height={150} /> Return</Button>
                <Button onClick={() => handleComplete()} variant={'secondary'} style={{ height: '27px' }} className='text-secondary w-3/4 rounded-lg' type="submit">Complete</Button>
            </div>
          </form>
        </Form>
    )
