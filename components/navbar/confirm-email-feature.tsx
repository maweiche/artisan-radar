...
const ConfirmEmailDialog: React.FC<DialogProps> = ({ _isOpen }) => {
    ...
    const handleEmailCheck = useCallback(async () => {
        try {
            ...
            const result = await checkUserEmail({
                variables: {
            ...
            if (result.data && result.data.checkEmail) {
                console.log('User updated successfully:', result.data.checkEmail);
    ...
    }, [userWallet, user, checkAuth, checkUserEmail, formData, router]);
